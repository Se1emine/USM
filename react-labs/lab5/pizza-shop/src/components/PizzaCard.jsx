 ...
 * @param {Function} props.onDelete - Функция удаления пиццы
 * @param {Function} props.onEdit - Функция редактирования пиццы
 * @returns {JSX.Element} JSX элемент карточки пиццы
 */
function PizzaCard({ pizza, onDelete, onEdit }) {
   ...
   */
  const handleDelete = () => {
    if (window.confirm(`Вы уверены, что хотите удалить "${pizza.name}"?`)) {
      onDelete(pizza.id);
    }
  };
    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
      ...
      <div className="p-4">
        <Link to={`/product/${pizza.id}`}>
          <h2 className="text-xl font-semibold text-gray-800 mb-2 hover:text-orange-600 transition-colors cursor-pointer">
            {pizza.name}
          </h2>
        </Link>
        ...
        <div className="space-y-2">
          ...
          {/* Кнопки управления (показываем только если переданы обработчики) */}
          {(onEdit || onDelete) && (
            <div className="flex gap-2">
              ...
              {onDelete && (
                <button
                  ...
                  className="flex-1 bg-red-500 text-white py-2 px-3 rounded-md hover:bg-red-600 transition-colors font-medium text-sm"
                >
                  Удалить
                </button>
              )}
